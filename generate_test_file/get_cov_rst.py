...
for gcov_test_dir in gcov_test_dirs:
    if os.path.isdir(IN_DIR + "/" + gcov_test_dir):
        ...
        total_branch_cnt -= 2
        hit_branch_cnt -= 2
        hit_branch_percent = (hit_branch_cnt / total_branch_cnt) * 100
        print("-" * 50)
        print(f"{gcov_test_dir}==={branch_str}==={hit_branch_cnt}={total_branch_cnt}={hit_branch_percent}%=")
        print("-" * 50)
